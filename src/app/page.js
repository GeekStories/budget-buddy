...
export default function Home() {
  ...
  return (
    <div className="flex flex-col gap-4 m-auto w-1/3 p-4 bg-slate-200 rounded-lg">
      <div className="flex gap-1 w-full bg-slate-100">
        <div className="w-full h-full bg-white min-h-96 text-center">
          {ExpensesList.map((expense, index) => (
            <div
              key={`expense_${index}`}
              className="text-red-500 text-xl font-medium hover:line-through hover:cursor-pointer"
        ...
        <div className="w-full h-full bg-white min-h-96 text-center">
          {IncomeList.map((income, index) => (
            <div
              key={`income_${index}`}
              className="text-green-500 text-xl font-medium hover:line-through hover:cursor-pointer"
      ...
      <h1 className="text-black text-center text-3xl border-b-2 border-black pb-4">
